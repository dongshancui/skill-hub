...
> 我现在还是在做 harness engineering，我在想设计和开发两个阶段的文档应该是什么，设计阶段需要把所有具体的 sql + 代码示例都搞定吗，开发阶段只负责任务拆分和实现验证。还是另一个方式，设计阶段只把核心数据库设计，同步流程和关键 sql 以及验收标准给定，然后开发过程自行进行代码编写，验证修复实现等等呢。哪个方式更合理，更加符合业务规范，或者更高效？
